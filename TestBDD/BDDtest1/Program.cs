...
namespace ConsoleApplication2
{
    ...
    class Program
    {
        ...
        private const string SqlConnectionString = @"Server=.\SQLExpress;Database=Exo2;Trusted_Connection=Yes";
        static void SauvegarderBdd(Operation operationASauvegarder)
        {
            SqlConnection connexion = new SqlConnection(SqlConnectionString);
            connexion.Open();

            SqlCommand command = new SqlCommand(@"INSERT INTO Operation(Operateur, OperandeDroite, OperandeGauche) VALUES (@operateur, @OperandeDroite, @OperandeGauche)", connexion);
            command.Parameters.AddWithValue("@operateur", TraduireTypeOperateurEnOperateurBDD(operationASauvegarder.Operateur));
            command.Parameters.AddWithValue("@OperandeDroite", operationASauvegarder.OperandeDroite);
            command.Parameters.AddWithValue("@OperandeGauche", operationASauvegarder.OperandeGauche);
